...
export default class extends Controller {
	...
	static values = {
		readonly: { type: Boolean, default: false }, // Nouvelle valeur pour le mode lecture seule
	};

	connect() {
		// Manually initialize Flowbite components (dropdowns, tooltips) within this new content
		// Only if not in readonly mode, or if you want tooltips/dropdowns to appear even in readonly (unlikely)
		...
		this.editor = new Editor({
			...
			editable: !this.readonlyValue, // Définir l'état d'édition basé sur la valeur readonly
			editorProps: {
				attributes: {
					class:
			...
			onUpdate: ({ editor }) => {
				// Mettre à jour le champ caché seulement si l'éditeur n'est PAS en lecture seule
				if (!this.readonlyValue) {
	...
	// -- Tiptap Actions (ces méthodes seront appelées uniquement si les boutons sont affichés) --
	// Pour des raisons de propreté, vous pourriez ajouter des checks `if (!this.readonlyValue)`
	// à chaque action, mais c'est redondant si les boutons sont cachés.
